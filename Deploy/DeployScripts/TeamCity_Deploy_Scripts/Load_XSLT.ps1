...
[String]$FullFilePath= $DeployBackupPath +"\TCConfiguration_" + $CurrentDate + "_" + $BuildNumber+"\XSLT\Full_XSLT.txt",
[String]$DeltaFilePath= $DeployBackupPath +"\TCConfiguration_" + $CurrentDate + "_" + $BuildNumber+"\XSLT\Delta_XSLT.txt",
[String]$BatFilePath= $DeployBackupPath +"\TCConfiguration_" + $CurrentDate + "_" + $BuildNumber+"\XSLT\Import_XSLT.bat",
[String]$LogFilePath= $DeployBackupPath +"\TCConfiguration_" + $CurrentDate + "_" + $BuildNumber+"\XSLT\Log_XSLT.txt",
[String]$ErrorFilePath= $DeployBackupPath +"\TCConfiguration_" + $CurrentDate + "_" + $BuildNumber+"\XSLT\ErrorLog_XSLT.txt",
[String]$GitFileChangePath="SourceCode/Configuration/XSLT/",
...
if ($Found){
write-host "XSLT files are modified !!"
# Remote Server.
...
if( $LASTEXITCODE -eq 0 -and [string]::IsNullOrEmpty($ErrorFound)) {
	$returncode="Success"
	write-host "XSLT Imported Successfully !!"
} else {
	$returncode="Failed"
	write-host "XSLT import Failed"
}
$returncode
...
write-host "No Modifications to XSLT !!"
}
